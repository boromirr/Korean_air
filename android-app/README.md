...
### 0.1.1: 달력에서 이전·다음 달 이동

달력 제목의 **‹ / ›** 버튼을 누르면 같은 노선과 좌석 등급으로 이전·다음 달을 바로 조회합니다. 월 선택 목록도 함께 변경됩니다. 조회하는 동안 중복 클릭을 막고, 오늘부터 360일 범위를 벗어나는 방향은 비활성화합니다. 실패 화면에도 이동 버튼을 유지합니다. 기존 0.1.0과 같은 키로 서명하고 버전 코드를 올려 업데이트 설치할 수 있게 했습니다. 실기기 화면 동작과 대한항공 실조회는 아직 미검증입니다.

### 0.1.2: 좌석 등급별 색상

일반석은 파랑, 프리미엄석은 초록, 프레스티지석은 보라, 일등석은 주황으로 표시합니다. 전체 등급 달력에서는 일반·프리미엄·프레스티지·일등석 순서의 고정된 위치에 가능한 등급의 색 점을 모두 표시합니다. 날짜별 상세와 등급 선택 목록도 같은 색을 사용합니다. 프리미엄·프레스티지 승급은 해당 등급 색의 ◇ 표시와 ‘승급’ 문구를 함께 사용하며, 보너스 달력의 가능 표시로 합산하지 않습니다. 일등석은 원본 자료의 보너스·승급 통합 표시를 유지합니다. 색상 범례와 화면 읽기용 등급 설명을 제공합니다. 기존 앱과 같은 서명, 버전 코드 3으로 빌드합니다.
